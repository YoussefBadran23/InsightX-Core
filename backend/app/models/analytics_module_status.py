...
class AnalyticsModuleStatus(UUIDMixin, TimestampMixin, Base):
    ...
    After CSV validation, the system writes 39 rows — one for each module —
    ...
    missing_required_columns: Mapped[list | None] = mapped_column(
        JSONB, nullable=True
    )

    # JSONB list of missing optional column names (nice-to-have)
    missing_optional_columns: Mapped[list | None] = mapped_column(
        JSONB, nullable=True
    )
