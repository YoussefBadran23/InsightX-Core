...
class UploadJob(UUIDMixin, TimestampMixin, Base):
    ...
    # Native currency of the CSV's monetary values (ISO 4217, e.g. 'USD','BRL','EGP').
    # Set in two ways:
    #   1. At /upload/confirm time if the user explicitly picked one from the modal.
    #   2. In stage3_preprocess from the mode of the CSV's `currency` column.
    # Falls back to 'USD' if neither path determines a value. Frontend reads this
    # so values can be FX-converted into the user's preferred display currency.
