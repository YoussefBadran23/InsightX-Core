...
def list_customers(
    ...
    # Aggregates over the FULL filtered set — drive the cards on the page.
    # One SQL round-trip; no order_by here so the engine can skip the sort.
    agg = query.with_entities(
        func.count(Customer.id),
        func.coalesce(func.avg(Customer.lifetime_value), 0),
        func.count(case((Customer.ai_segment == "vip_champion", 1))),
        func.count(case((Customer.ai_segment == "at_risk", 1))),
    ).one()
    total = int(agg[0] or 0)
    avg_ltv = Decimal(agg[1] or 0)
    vip_count = int(agg[2] or 0)
    at_risk_count = int(agg[3] or 0)
