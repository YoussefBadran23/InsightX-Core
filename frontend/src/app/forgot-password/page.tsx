...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('success');
      setMessage(t('fp_successMsg', language));
    } catch (err: any) {
      setStatus('error');
      setMessage(err.response?.data?.detail || t('fp_errorFallback', language));
    }
  };
      ...
      <main className="relative z-10 flex-1 flex w-full flex-col items-center justify-center px-4 sm:px-6 pt-24 pb-12">
        <div className="w-full max-w-[440px] rounded-2xl bg-white/90 dark:bg-[#16162a]/90 backdrop-blur-md border border-gray-200 dark:border-white/5 p-8 shadow-2xl sm:p-10">
          ...
          <div className="mb-8 text-center">
            <h1 className="text-gray-900 dark:text-white text-[28px] font-bold leading-tight tracking-tight">
              {t('fp_title', language)}
            </h1>
            <p className="mt-2 text-slate-500 dark:text-slate-400 text-sm">
              {t('fp_subtitle', language)}
            </p>
          </div>
          ...
          {status === 'success' ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <label className="flex flex-col gap-2">
                <span className="text-gray-900 dark:text-slate-200 text-sm font-medium leading-normal">
              ...
              >
                {status === 'loading' ? t('fp_sending', language) : t('fp_sendLink', language)}
              </button>
            </form>
          )}
