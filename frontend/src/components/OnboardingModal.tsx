...
export function OnboardingModal({ onComplete }: OnboardingModalProps) {
  ...
  useEffect(() => {
    uploadApi.getSchemaColumns().then(res => setSchemaColumns(res.data)).catch(console.error);
  }, []);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await uploadApi.confirmMappings({ sniff_id: sniffId, confirmed_mappings });
