...
export interface UploadJob {
  ...
  /** ISO 4217 native currency of the CSV (detected in stage3 or chosen at upload). */
...
export interface KpiSummary {
  ...
  /**
   * Native currency of the latest completed upload (ISO 4217, e.g. 'USD','BRL').
   * Pass as the `from` argument to `formatMoney(value, displayCurrency, { from })`
   * so values FX-convert into the user's preferred display currency.
   */
