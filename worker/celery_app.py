...
celery_app.conf.update(
    ...
    task_acks_late=False,              # Ack immediately — higher throughput for analytics chord
    worker_prefetch_multiplier=4,      # Fetch ahead so workers stay busy between tasks
    task_soft_time_limit=120,          # Warn after 2 min (analytics shouldn't run longer)
    task_time_limit=180,               # Hard-kill after 3 min — prevents hanging tasks

    # Redis memory hygiene
    result_expires=1800,               # Clear results after 30 min (saves Redis RAM)
