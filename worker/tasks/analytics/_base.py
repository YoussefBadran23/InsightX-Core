...
def coerce_numeric(series: pd.Series) -> pd.Series:
    ...
    Returns NaN for anything unparseable.  Also handles the case where dirty
    CSVs with duplicate column names cause ``df[col]`` to return a DataFrame —
    we take the first column so coercion always receives a 1-D Series.
    """
...
def coerce_date(series: pd.Series) -> pd.Series:
    ...
    Handles the case where a DataFrame slice is passed instead of a Series
    (happens when dirty CSVs contain duplicate column names).  We take the
    first column in that case so downstream coercion always receives a 1-D
    Series and ``pd.to_datetime`` never tries to assemble from unit-mappings.
    """
    if isinstance(series, pd.DataFrame):
        series = series.iloc[:, 0]
