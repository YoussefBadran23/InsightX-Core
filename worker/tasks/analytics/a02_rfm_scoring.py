...
QUESTION = "Who are my most valuable customers — and which ones are slipping away?"
...
def run(df: pd.DataFrame) -> dict[str, Any]:
        ...
        return {
            **build_payload(
                ...
                headline=build_headline(
                    value=0, label="champions identified",
                    period="no data",
                ),
                fallback_bullets=[
                    "Not enough customer history yet — RFM segmentation needs "
                    "at least 5 distinct customers with order dates.",
                    "Once you have more data, this card highlights Champions, "
                    "At Risk, and Lost segments so you can target each tier.",
                    "Tip: even a month of orders gives a usable first cut — "
                    "scores get sharper as the dataset grows.",
                ],
                suggested_actions=[
    ...
    # `top_customers` originally held only the top 20 by spend. That broke
    # stage6_finalize._denormalize_rfm, which uses this list to write
    # ai_segment / rfm_segment / rfm_score back onto every customer row —
    # the dashboard ended up with 99% of customers showing "no segment".
    # We now emit the FULL RFM-scored table (sorted by monetary so the
    # chart's slice-first-N still surfaces the heaviest spenders). The
    # consumers (A02RFM3DChart, segmentation page scatter) handle thousands
    # of points fine, and at ~9 fields per customer JSONB stays small.
    top = rfm.sort_values("monetary", ascending=False)
    ...
    by_seg = {s["segment"]: s for s in segments_out}
    champions = by_seg.get("Champions", {"count": 0, "revenue": 0.0, "rev_pct": 0.0})
    at_risk = by_seg.get("At Risk", {"count": 0, "revenue": 0.0, "rev_pct": 0.0})
    cannot_lose = by_seg.get("Cannot Lose Them", {"count": 0, "revenue": 0.0})
    n_customers = int(len(rfm))

    headline = build_headline(
        value=int(champions["count"]),
        label="champions driving your revenue",
        period=f"{fmt_int(n_customers)} customers scored",
    )

    bullets: list[str] = []
    if champions["count"] > 0:
        bullets.append(
            f"{fmt_int(champions['count'])} Champions ({fmt_money(champions['revenue'])}, "
            f"{champions['rev_pct']:.0f}% of revenue) — keep them happy first."
        )
    else:
        bullets.append(
            "No Champions tier yet — nobody scores 4+ on recency, frequency, AND "
            "monetary. Focus on repeat-purchase incentives to grow this tier."
        )

    at_risk_count = int(at_risk["count"]) + int(cannot_lose["count"])
    at_risk_rev = float(at_risk["revenue"]) + float(cannot_lose["revenue"])
    if at_risk_count > 0:
        bullets.append(
            f"{fmt_int(at_risk_count)} customers At Risk or quiet "
            f"({fmt_money(at_risk_rev)} in past revenue) — win-back campaign now."
        )
    elif n_customers > 0:
        bullets.append(
            "Healthy retention: no customers in At Risk or Cannot Lose tiers right now."
        )

    top_seg = segments_out[0] if segments_out else None
    if top_seg and top_seg["segment"] not in ("Champions",):
        bullets.append(
            f"Biggest segment is {top_seg['segment']} ({top_seg['pct']:.0f}% of customers) — "
            f"messaging should speak to them first."
        )
    else:
        bullets.append(
            "Run a 10% Champions-only offer this quarter — they convert at the "
            "highest rate and have the lowest acquisition cost."
        )

    actions = [
        action("View Champions", kind="primary",
               deeplink="/dashboard/segmentation?segment=Champions", icon="arrow"),
    ]
    if at_risk_count > 0:
        actions.append(action(
            "Win back at-risk", kind="warning",
            deeplink="/dashboard/segmentation?segment=at-risk", icon="arrow",
        ))
    actions.append(action(
        "Export segments CSV", kind="secondary", deeplink=None, icon="download",
    ))
