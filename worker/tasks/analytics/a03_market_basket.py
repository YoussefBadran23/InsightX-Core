...
def run(df: pd.DataFrame) -> dict[str, Any]:
    ...
    if n_orders == 0:
        return {
            **build_payload(
                question=QUESTION,
                headline=build_headline(value=0, label="product pairs found", period="no data"),
                fallback_bullets=[
                    "No order data yet — market basket needs orders with at least 2 products each.",
                    "Once multi-item orders exist, this card surfaces top product pairs with lift, support, and confidence.",
                    "Use these pairs to suggest cross-sells at checkout — easiest revenue lift in e-commerce.",
                ],
                suggested_actions=[
                    action("Upload order data", kind="primary",
    ...
    avg_size = sum(sizes) / n_orders
    n_pairs = len(pair_rules)
    top_pair = pair_rules[0] if pair_rules else None
    top_pair_lift = float(top_pair.get("lift", 0)) if top_pair else 0

    headline = build_headline(
        value=n_pairs,
        label="strong product pairs found",
        period=f"{fmt_int(n_orders)} orders analysed",
    )

    bullets: list[str] = []
    if top_pair:
        names = top_pair.get("antecedent_names") or top_pair.get("antecedent_name") or top_pair.get("antecedent")
        cons = top_pair.get("consequent_names") or top_pair.get("consequent_name") or top_pair.get("consequent")
        bullets.append(
            f"Strongest pair: {str(names)[:30]} → {str(cons)[:30]} "
            f"(lift {top_pair_lift:.2f}× — bought together {top_pair_lift:.1f}× more than random)."
        )
    else:
        bullets.append(f"Found {n_pairs} co-occurring product pairs from {fmt_int(n_orders)} orders.")

    multi_pct = round(n_multi / n_orders * 100, 1)
    if multi_pct >= 30:
        bullets.append(
            f"{multi_pct}% of orders contain multiple items (avg basket size {avg_size:.1f}) — "
            f"strong cross-sell foundation already exists."
        )
    else:
        bullets.append(
            f"Only {multi_pct}% of orders contain multiple items — big opportunity to grow "
            f"basket size with recommended-product widgets."
        )

    if top_pair_lift >= 3:
        bullets.append(
            "Wire your top 5 pairs into the product-page 'Frequently bought with' widget — "
            "high-lift pairs convert >3× better than generic suggestions."
        )
    else:
        bullets.append(
            "Test bundling your top 3 pairs as a 'Save 5%' combo offer — easy way to "
            "turn co-occurrence into incremental revenue."
        )

    actions = [
        action("View all pairs", kind="primary",
               deeplink="/dashboard/analytics?focus=A03", icon="arrow"),
        action("Export pairs CSV", kind="secondary", deeplink=None, icon="download"),
    ]
