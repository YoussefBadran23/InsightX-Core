...
QUESTION = "Which products drive most of my revenue — and which are dead weight?"
...
def run(df: pd.DataFrame) -> dict[str, Any]:
    ...
    if df.empty or df["product_id"].nunique() == 0:
        return {
            **build_payload(
                question=QUESTION,
                headline=build_headline(value=0, label="hero products (Tier A)",
                                        period="no data"),
                fallback_bullets=[
                    "No product data yet — ABC classification can't rank SKUs "
                    "without a product_id + revenue column.",
                    "Once your products are tagged, this card surfaces the top "
                    "20% driving most revenue and flags the long tail.",
                    "Tip: most stores follow the 80/20 rule — 20% of SKUs earn "
                    "80% of revenue. Knowing yours is power.",
                ],
                suggested_actions=[
    ...
    # ── Per-product output (FULL list, sorted by revenue desc) ──────────────
    # Previously this was sliced to the top 50 + tier-transition rows. That
    # broke stage6_finalize._denormalize_abc, which copies `tier` onto every
    # product row — only the 50 in the JSON got tagged; the rest stayed NULL
    # and showed up as "Unranked" on the dashboard even when they had revenue.
    #
    # Emit ALL ranked products. The frontend's A07 chart only takes
    # `products.slice(0, 25)` anyway, so its render path is unchanged. JSONB
    # payload for 50k SKUs is ~3MB — well within Postgres jsonb limits and
    # only read once by stage6, never per request.
    products_out = [
        {
    ...
    headline = build_headline(
        value=int(tier_a["products"]),
        label=f"hero products driving {tier_a['revenue_pct']:.0f}% of revenue",
        period=f"{fmt_int(n_products)} SKUs ranked",
    )

    bullets: list[str] = []
    bullets.append(
        f"Top {int(tier_a['products_pct'])}% of SKUs ({fmt_int(tier_a['products'])} products) "
        f"earn {tier_a['revenue_pct']:.0f}% of revenue — protect these hero SKUs."
    )
    if follows:
        bullets.append(
            f"Classic 80/20 split: top 20% of products = {top20_share:.0f}% of "
            f"revenue — healthy concentration."
        )
    elif top20_share < 70.0:
        bullets.append(
            f"Revenue is unusually spread out — top 20% only earn {top20_share:.0f}% "
            f"(below 80/20). Means lots of mid-tier products carry the business."
        )
    else:
        bullets.append(
            f"Heavy concentration — top 20% of SKUs earn {top20_share:.0f}% of revenue. "
            f"Lose one hero product and the business takes a real hit."
        )
    if tier_c["products"] > 0:
        bullets.append(
            f"{fmt_int(tier_c['products'])} long-tail SKUs ({tier_c['products_pct']:.0f}% of catalog) "
            f"earn only {tier_c['revenue_pct']:.1f}% — review for delisting."
        )
    else:
        bullets.append(
            "No long-tail dead weight detected — every SKU is contributing meaningfully."
        )

    actions = [
        action("View hero products", kind="primary",
               deeplink="/dashboard/products?tier=A", icon="arrow"),
    ]
    if tier_c["products"] > 0:
        actions.append(action(
            "Review long tail", kind="warning",
            deeplink="/dashboard/products?tier=C", icon="arrow",
        ))
    actions.append(action(
        "Export ABC report", kind="secondary", deeplink=None, icon="download",
    ))
