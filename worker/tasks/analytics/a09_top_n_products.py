...
def run(df: pd.DataFrame) -> dict[str, Any]:
    ...
    if df.empty:
        return {
            **build_payload(
                question=QUESTION,
                headline=build_headline(value=0, label="top sellers", period="no data"),
                fallback_bullets=[
                    "No product-level revenue data yet — upload sales rows "
                    "with product_id and total_amount to rank top sellers.",
                    "Once data lands, you'll see top 20 products by revenue, "
                    "orders, AOV and margin side-by-side.",
                    "Tip: include product_name + category for human-readable rankings.",
                ],
                suggested_actions=[
    ...
    top1 = top_by_revenue[0] if top_by_revenue else None
    top1_aov = top_by_aov[0] if top_by_aov else None
    top1_margin = (top_by_margin or [None])[0] if has_cost else None

    headline = build_headline(
        value=round(float(prod["revenue"].sum()), 2),
        label=f"revenue from {int(len(prod))} products",
        period="All time",
    )

    bullets: list[str] = []
    # 1 — Best seller by revenue
    if top1:
        nm = top1.get("name") or top1.get("product_id") or "Unknown"
        bullets.append(
            f"Best seller: {str(nm)[:40]} — {fmt_money(top1['revenue'])} "
            f"across {top1['orders']} orders."
        )
    # 2 — AOV vs revenue tradeoff
    if top1_aov and top1 and top1_aov.get("product_id") != top1.get("product_id"):
        nm = top1_aov.get("name") or top1_aov.get("product_id") or "Unknown"
        bullets.append(
            f"Premium pick: {str(nm)[:40]} has the highest AOV "
            f"({fmt_money(top1_aov['aov'])}) — small volume but high value per sale."
        )
    elif top1_margin and top1 and top1_margin.get("product_id") != top1.get("product_id"):
        nm = top1_margin.get("name") or top1_margin.get("product_id") or "Unknown"
        bullets.append(
            f"Margin champion: {str(nm)[:40]} contributes "
            f"{fmt_money(top1_margin.get('margin', 0))} in margin — "
            f"protect this SKU's stock."
        )
    else:
        bullets.append(
            f"Top 5 products carry "
            f"{sum(r['revenue'] for r in top_by_revenue[:5]) / max(float(prod['revenue'].sum()), 1) * 100:.0f}% "
            f"of total revenue — concentration risk to monitor."
        )

    # 3 — Bottom / delisting candidate
    if bottom_by_revenue and len(prod) > 10:
        n_bottom_zero = int((prod["revenue"] == 0).sum())
        if n_bottom_zero >= 3:
            bullets.append(
                f"{n_bottom_zero} products had zero revenue this period — "
                f"consider delisting or promotional bundling."
            )
        else:
            worst = bottom_by_revenue[0]
            nm = worst.get("name") or worst.get("product_id") or "Unknown"
            bullets.append(
                f"Slowest mover: {str(nm)[:40]} with only "
                f"{fmt_money(worst['revenue'])} — review or bundle."
            )
    else:
        bullets.append(
            "Reorder the top 3 sellers before they go out of stock — "
            "stockouts are silent revenue killers."
        )

    actions = [
        action("View top 20", kind="primary",
               deeplink="/dashboard/products?sort=revenue", icon="arrow"),
        action("Restock recommendations", kind="positive",
               deeplink="/dashboard/products?filter=top10&action=restock", icon="arrow"),
        action("Export product CSV", kind="secondary",
