...
QUESTION = "How many orders are stuck and need my attention right now?"
...
def run(df: pd.DataFrame) -> dict[str, Any]:
        ...
        return {
            **build_payload(
                ...
                headline=build_headline(
                    value=0, label="orders need attention",
                    period="no data",
                ),
                fallback_bullets=[
                    "No `status` column found — order operations are flying blind.",
                    "Tag every order with a status (pending / shipped / delivered / "
                    "cancelled / refunded) to unlock fulfillment analytics.",
                    "Shopify/WooCommerce exports already include this; manual "
                    "spreadsheets usually don't.",
                ],
                suggested_actions=[
                    action("Add status column", kind="primary",
    ...
    success_pct = round(success_n / n_orders * 100, 1)
    cancel_pct = round(cancel_n / n_orders * 100, 1)
    refund_pct = round(refund_n / n_orders * 100, 1)
    progress_pct = round(progress_n / n_orders * 100, 1)

    # Headline = in-progress + failure count (orders needing attention)
    needs_attention = progress_n + cancel_n + refund_n
    headline = build_headline(
        value=needs_attention,
        label="orders need attention",
        trend_pct=None,  # no period comparison for headline; trend lives in monthly_trend
        period="Right now",
    )

    bullets: list[str] = []
    # 1 — Success rate framing
    if success_pct >= 90:
        bullets.append(
            f"Healthy: {success_pct}% of {fmt_int(n_orders)} orders completed — "
            f"only {fmt_int(needs_attention)} need attention."
        )
    elif success_pct >= 70:
        bullets.append(
            f"OK: {success_pct}% completion rate, but {fmt_int(needs_attention)} "
            f"orders are pending or have failed — review the failure column."
        )
    else:
        bullets.append(
            f"Warning: only {success_pct}% of orders complete successfully — "
            f"{fmt_int(needs_attention)} orders flag for review."
        )

    # 2 — Cancel/refund-focused
    if cancel_pct >= 5:
        bullets.append(
            f"{cancel_pct}% cancellation rate is high — likely root cause: "
            f"out-of-stock at checkout or pricing errors."
        )
    elif refund_pct >= 3:
        bullets.append(
            f"Refund rate of {refund_pct}% suggests quality or delivery issues — "
            f"check return reasons by product category."
        )
    elif progress_pct >= 15:
        bullets.append(
            f"{progress_pct}% of orders are still in-progress — investigate "
            f"any stuck longer than 7 days."
        )
    else:
        bullets.append(
            f"Cancellations at {cancel_pct}% and refunds at {refund_pct}% — "
            f"both within healthy bounds (<5%)."
        )

    # 3 — Revenue at risk action
    if revenue_at_risk and revenue_at_risk > 0:
        bullets.append(
            f"{fmt_money(revenue_at_risk)} of revenue is tied to unresolved "
            f"orders — resolve to convert it into actual cash flow."
        )
    else:
        bullets.append(
            "Set up automated alerts for orders in 'pending' >48h so nothing "
            "gets lost in the shuffle."
        )

    actions = [
        action("Review stuck orders", kind="primary",
               deeplink="/dashboard/analytics?focus=A11&filter=in_progress",
               icon="arrow"),
    ]
    if cancel_n > 0:
        actions.append(action(
            "Investigate cancellations", kind="warning",
            deeplink="/dashboard/analytics?focus=A11&filter=cancelled", icon="arrow",
        ))
    actions.append(action(
        "Export status report", kind="secondary", deeplink=None, icon="download",
    ))
