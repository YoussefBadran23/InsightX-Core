...
def run(df: pd.DataFrame) -> dict[str, Any]:
    ...
    if df.empty:
        return {
            **build_payload(
                question=QUESTION,
                headline=build_headline(value=0, label="given in discounts", period="no data"),
                fallback_bullets=[
                    "No discount data yet — promotions can't be measured "
                    "without a discount_amount or coupon column.",
                    "Once tagged, this card compares discounted vs full-price "
                    "orders and surfaces which discounts pay back.",
                    "Tip: track every promo separately so you can A/B test "
                    "future campaigns.",
                ],
                suggested_actions=[
                    action("Tag a discount column", kind="primary",
    ...
    total_disc_amount = float(summary.get("total_discount") or 0.0)
    disc_pct_gross = float(summary.get("discount_pct_of_gross") or 0.0)
    discounted_share_pct = float(summary.get("discounted_pct") or 0.0)
    avg_disc_pct = float(summary.get("avg_discount_pct") or 0.0)

    aov_ratio = None
    if comparison and comparison.get("discounted") and comparison.get("full_price"):
        d_aov = float(comparison["discounted"]["aov"] or 0)
        f_aov = float(comparison["full_price"]["aov"] or 0)
        aov_ratio = (d_aov / f_aov) if f_aov > 0 else None

    headline = build_headline(
        value=round(total_disc_amount, 2),
        label="given in discounts",
        trend_pct=None,
        period="All time",
    )

    bullets: list[str] = []
    # 1 — Headline framing of discount share
    bullets.append(
        f"You gave away {fmt_money(total_disc_amount)} "
        f"({disc_pct_gross:.1f}% of gross revenue) across "
        f"{int(summary.get('n_discounted') or 0):,} discounted orders."
    )
    # 2 — AOV ratio insight (do discounts actually grow basket size?)
    if aov_ratio is not None:
        if aov_ratio >= 1.10:
            bullets.append(
                f"Discounted orders have {(aov_ratio - 1) * 100:.0f}% bigger AOV "
                f"than full-price — promotions are pulling people into larger baskets."
            )
        elif aov_ratio <= 0.85:
            bullets.append(
                f"Discounted AOV is {(1 - aov_ratio) * 100:.0f}% LOWER than full-price — "
                f"promotions are eroding margin without growing basket size."
            )
        else:
            bullets.append(
                f"Discounted vs full-price AOV is similar ({aov_ratio:.2f}× ratio) — "
                f"promotions are neutral; ROI is in customer acquisition, not basket growth."
            )
    else:
        bullets.append(
            f"Average discount: {avg_disc_pct:.1f}% off — review whether your typical "
            f"promo is moving the needle or just chipping at margin."
        )
    # 3 — Recommendation
    if discounted_share_pct >= 40:
        bullets.append(
            f"{discounted_share_pct:.0f}% of orders are discounted — discount "
            f"fatigue risk: customers may now wait for promos to buy."
        )
    elif total_disc_amount > 0:
        bullets.append(
            "Run a hold-out test: temporarily pause one promo and measure revenue "
            "delta — only way to know real incremental lift."
        )
    else:
        bullets.append(
            "No discounts tagged — if you ran promos, ensure the discount_amount "
            "column is populated so future analysis is precise."
        )

    actions = [
        action("Inspect promo categories", kind="primary",
               deeplink="/dashboard/analytics?focus=A12", icon="arrow"),
        action("Plan next promo", kind="positive",
               deeplink="/dashboard/forecasting?action=simulate-promo", icon="arrow"),
        action("Export discount report", kind="secondary",
