...
QUESTION = "Is my business growing month over month?"
...
def run(df: pd.DataFrame) -> dict[str, Any]:
    ...
    if df.empty:
        return {
            **build_payload(
                question=QUESTION,
                headline=build_headline(value=0, label="monthly growth rate",
                                        period="no data"),
                fallback_bullets=[
                    "No revenue data yet — growth rates need at least 2 "
                    "months of orders to compute.",
                    "Once data accrues, this card shows MoM/QoQ/YoY growth "
                    "and your compound monthly growth rate (CMGR).",
                    "Tip: a 5% CMGR doubles revenue in ~14 months — small "
                    "compounds matter.",
                ],
                suggested_actions=[
    ...
    headline = build_headline(
        ...
        trend_pct=cmgr_pct,
        period=f"{n_months} months · {monthly['_period_m'].iloc[0]} → {monthly['_period_m'].iloc[-1]}",
    )
    ...
    if trend_label == "growing":
        bullets.append(
            f"Revenue is compounding at {cmgr_pct:.1f}%/month — at this rate "
            f"you'll double in {(0.693 / max(cmgr_pct / 100, 0.001)):.0f} months."
        )
    elif trend_label == "shrinking":
        bullets.append(
            f"Revenue is contracting {cmgr_pct:.1f}%/month — total drop of "
            f"{abs(total_growth):.0f}% from first to last month. Diagnose now."
        )
    else:
        bullets.append(
            f"Revenue is flat: CMGR {cmgr_pct:+.1f}%/month over {n_months} months. "
            f"Stability is fine but plateau is the silent killer."
        )

    # 2 — Best vs worst month callout
    if best_mom and worst_mom:
        bullets.append(
            f"Best month: {best_mom['period']} (+{best_mom['growth_pct']:.0f}% MoM). "
            f"Worst: {worst_mom['period']} ({worst_mom['growth_pct']:+.0f}%) — what happened?"
        )
    elif best_mom:
        bullets.append(
            f"Best growth month: {best_mom['period']} (+{best_mom['growth_pct']:.0f}% MoM)."
        )

    # 3 — YoY callout if we have it, else recommendation
    if yoy_summary and yoy_summary.get("yoy_pct") is not None:
        yoy = float(yoy_summary["yoy_pct"])
        if yoy > 0:
            bullets.append(
                f"Year over year you're up {yoy:.0f}% "
                f"({fmt_money(yoy_summary['current_year_revenue'])} vs "
                f"{fmt_money(yoy_summary['prior_year_revenue'])} prior 12mo)."
            )
        else:
            bullets.append(
                f"Year over year revenue is down {abs(yoy):.0f}% — "
                f"this isn't a blip, it's a trend. Time to reassess strategy."
            )
    elif len(bullets) < 3:
        if cmgr_pct < 1.0:
            bullets.append(
                "Set a 12-month CMGR target of at least 3%/mo — that's "
                "≈40% annual growth, the floor for compounding businesses."
            )
        else:
            bullets.append(
                "Tip: 12+ months of data unlocks year-over-year comparison "
                "and Prophet-based forecasting (A15)."
            )

    actions = [
        action("Open growth analysis", kind="primary",
               deeplink="/dashboard/analytics?focus=A13", icon="arrow"),
    ]
    if trend_label == "shrinking":
        actions.append(action(
            "Diagnose decline", kind="danger",
            deeplink="/dashboard/analytics?focus=A13&split=cohort", icon="arrow",
        ))
    else:
        actions.append(action(
            "Forecast next 90 days", kind="info",
            deeplink="/dashboard/forecasting", icon="arrow",
        ))
    actions.append(action(
        "Export growth report", kind="secondary", deeplink=None, icon="download",
    ))
