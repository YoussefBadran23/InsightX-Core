...
def run(df: pd.DataFrame) -> dict[str, Any]:
    ...
    headline = build_headline(
        value=n_anom,
        label="unusual days flagged",
        period=f"Last {n_days} days analysed",
    )
    ...
    if n_anom == 0:
        bullets.append(
            f"Healthy: zero unusual days across {n_days} days of data — "
            f"your revenue is stable and predictable."
        )
    elif n_high > n_low:
        bullets.append(
            f"{n_high} spike day(s) detected — celebrate, then learn: "
            f"what drove them? Replay the playbook."
        )
    elif n_low > 0:
        bullets.append(
            f"{n_low} dip day(s) detected — investigate stockouts, site "
            f"outages, or competitor promos around those dates."
        )
    else:
        bullets.append(
            f"{n_anom} anomalies across {n_days} days "
            f"({n_anom / max(n_days, 1) * 100:.1f}% rate) — balanced spikes and dips."
        )

    # 2 — Most extreme day callout
    if most_extreme:
        kind = most_extreme.get("type", "high")
        dev = most_extreme.get("deviation_pct", 0)
        date_str = most_extreme.get("date", "")
        if kind == "high":
            bullets.append(
                f"Biggest outlier: {date_str} — revenue ran "
                f"{abs(dev):.0f}% above the expected range. Replicate what worked."
            )
        else:
            bullets.append(
                f"Biggest dip: {date_str} — revenue dropped "
                f"{abs(dev):.0f}% below the expected range. Was the site up? Stock OK?"
            )
    elif n_anom == 0:
        bullets.append(
            "When a spike or dip does happen, this card will surface it "
            "automatically — no need to scan tables."
        )
    else:
        bullets.append(
            "Click any flagged date to see the underlying orders and "
            "diagnose the cause."
        )
    ...
    if n_anom == 0:
        bullets.append(
            "Stable is good — your forecasting models will be more accurate. "
            "Watch for a sudden flag as an early-warning signal."
        )
    elif n_high >= 2 and n_low == 0:
        bullets.append(
            "Multiple positive spikes — turn them into a campaign playbook "
            "before the lift fades."
        )
    else:
        bullets.append(
            "Set up an email alert for any new anomaly — every 4 hours so "
            "nothing slips past your team."
        )

    actions: list[dict[str, Any]] = [
        action("Inspect flagged days", kind="primary",
               deeplink="/dashboard/analytics?focus=A16", icon="arrow"),
    ]
    if n_anom > 0:
        actions.append(action(
            "Configure alerts", kind="warning",
            deeplink="/dashboard/settings/alerts", icon="arrow",
        ))
    actions.append(action(
        "Export anomaly log", kind="secondary",
...
def _empty_result(warning: str) -> dict[str, Any]:
    return {
        **build_payload(
            question=QUESTION,
            headline=build_headline(value=0, label="unusual days flagged",
                                    period="no data"),
            fallback_bullets=[
                "Anomaly detection needs at least 30 days of daily revenue "
                "to learn what 'normal' looks like.",
                "Upload more history (3+ months ideal) to unlock spike-and-dip "
                "detection with rolling Z-score + IQR fences.",
                "Once active, the model flags any day that deviates 3 std "
                "devs from its 30-day baseline.",
            ],
            suggested_actions=[
                action("Upload more data", kind="primary",
