...
QUESTION = "How much will my customers be worth a year from now?"
...
def run(df: pd.DataFrame) -> dict[str, Any]:
    ...
    n_modeled = int(len(summary))
    horizon_90 = horizon_results.get(90, {})
    horizon_365 = horizon_results.get(365, {})

    headline = build_headline(
        value=round(avg_clv_365, 2),
        label="predicted CLV per customer · next 12 months",
        period=f"{fmt_int(n_modeled)} customers modeled",
    )

    bullets: list[str] = []
    bullets.append(
        f"Avg customer is projected to be worth {fmt_money(avg_clv_365)} over the "
        f"next 12 months · total cohort = {fmt_money(total_clv_365)}."
    )

    # Whale-skew callout
    if median_clv_365 > 0 and avg_clv_365 >= median_clv_365 * 1.5:
        ratio = avg_clv_365 / median_clv_365
        bullets.append(
            f"Distribution is whale-heavy: average is {ratio:.1f}× the median "
            f"({fmt_money(median_clv_365)}) — top spenders carry the forecast."
        )
    elif top_customers:
        top_clv = float(top_customers[0]["predicted_clv_365d"])
        bullets.append(
            f"Highest predicted CLV: {fmt_money(top_clv)} (customer "
            f"{str(top_customers[0]['customer_id'])[:12]}). Protect this account."
        )

    # 90d horizon as actionable short-term lens
    if horizon_90.get("avg_predicted_value", 0) > 0:
        bullets.append(
            f"Next 90 days alone: {fmt_money(horizon_90['total_clv'])} expected from "
            f"current customers · {horizon_90['avg_predicted_orders']:.1f} orders avg per customer."
        )
    else:
        bullets.append(
            "Tip: CLV assumes you keep your current acquisition mix. Bigger budgets "
            "shift the cohort; rerun after major campaigns."
        )

    actions = [
        action("View top predicted customers", kind="primary",
               deeplink="/dashboard/customers?sort=clv-desc", icon="arrow"),
        action("Compare to historical LTV", kind="info",
               deeplink="/dashboard/analytics?focus=A10", icon="arrow"),
        action("Export CLV report", kind="secondary",
    ...
    return {
        **build_payload(
        ...
        "warning": "; ".join(filter(None, warning_parts)) or None,
    }
...
def _empty(warning: str) -> dict[str, Any]:
    return {
        **build_payload(
            question=QUESTION,
            headline=build_headline(value=0, label="predicted CLV per customer",
                                    period="no data"),
            fallback_bullets=[
                "Not enough customer history for probabilistic CLV — need at "
                f"least {MIN_CUSTOMERS} customers and {MIN_REPEATERS} repeat buyers.",
                "Once eligible, this card predicts how much each customer will "
                "spend over the next 90 / 180 / 365 days.",
                "CLV is the single number that decides your acquisition budget — "
                "every customer worth more than your CAC is profitable.",
            ],
            suggested_actions=[
