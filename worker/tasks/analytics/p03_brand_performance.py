...
def run(df: pd.DataFrame) -> dict[str, Any]:
    ...
    top3_share = round(sum(b["rev_pct"] for b in by_brand[:3]), 1)
    ...
    if top_brand:
        bullets.append(
            f"Top brand: {top_brand['brand']} — {fmt_money(top_brand['revenue'])} "
            f"({top_brand['rev_pct']:.1f}% of revenue) across {top_brand['orders']} orders."
        )
    else:
        bullets.append(f"Tracking {n_brands} brands with revenue.")

    if top3_share >= 70:
        bullets.append(
            f"Top 3 brands carry {top3_share:.0f}% of revenue — heavy dependence. "
            f"Renegotiate hard, you have leverage."
        )
    elif top3_share >= 40:
        bullets.append(
            f"Top 3 brands at {top3_share:.0f}% — typical for SMB e-commerce. "
            f"Concentration: {hhi_label}."
        )
    else:
        bullets.append(
            f"Brand mix is diffuse: top 3 only {top3_share:.0f}% of revenue. "
            f"You're not over-reliant on any one supplier."
        )

    if hhi_label == "concentrated":
        bullets.append(
            "Diversify supplier base before a price hike or stock outage hits your "
            "top brand — one bad month from them = one bad month for you."
        )
    elif n_brands >= 10:
        bullets.append(
            f"Active brand portfolio of {n_brands} — focus marketing on the top 5 "
            f"and trim slow-movers to reduce inventory complexity."
        )
    else:
        bullets.append(
            "Add 2-3 complementary brands to expand catalog — diversifying suppliers "
            "reduces supply-chain risk."
        )

    actions = [
        action("View brand catalog", kind="primary",
               deeplink="/dashboard/products?group=brand", icon="arrow"),
        action("Export brand CSV", kind="secondary", deeplink=None, icon="download"),
    ]
...
def _empty(warning: str) -> dict[str, Any]:
    return {
        **build_payload(
            question=QUESTION,
            headline=build_headline(value=0, label="brands tracked", period="no data"),
            fallback_bullets=[
                "No brand data yet — add a brand column to your product/order export.",
                "Once tagged, this card surfaces top brands by revenue and identifies supplier concentration risk.",
                "Brand performance lets you negotiate harder with your biggest suppliers — leverage you can't use without data.",
            ],
            suggested_actions=[
                action("Add brand column", kind="primary",
