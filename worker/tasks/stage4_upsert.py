...
def stage4_upsert(self, job_id_str: str) -> str:
    ...
    try:
        ...
        if cust_records:
            ...
            stmt = insert(Customer).values(unique_records)
        ...
        if prod_records:
            ...
            for r in prod_records:
                ...
                seen_skus.add(sku)
                unique_records.append(r)
            stmt = insert(Product).values(unique_records)
            stmt = stmt.on_conflict_do_nothing()
            db.execute(stmt)
            db.commit()
        ...
        if order_records:
            ...
            # Conflict target matches uq_order_user_extid: (user_id, external_id).
            stmt = stmt.on_conflict_do_nothing(index_elements=["user_id", "external_id"])
            db.execute(stmt)
            db.commit()
