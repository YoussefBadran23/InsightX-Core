...
def _snapshot_kpis(db, user_id, job_id: str) -> None:
    """Compute today's KPI snapshot from ALL of the user's orders.

    Important: we intentionally do NOT filter by `upload_job_id`. The dashboard
    is meant to reflect the user's TOTAL state across every upload they've
    ever done. Filtering per-job caused the dashboard to show $0 whenever a
    re-uploaded CSV was de-duped at insert time (stage4's `ON CONFLICT DO
    NOTHING` skips repeat orders → 0 new rows for this job → 0 snapshot).
    ...
    WHEN matching against region codes (NA / EU / APAC / LATAM).

    `job_id` is still accepted for API compatibility but unused.
    """
    _ = job_id  # intentionally unused — see docstring
    db.execute(
        text("""
